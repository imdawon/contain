...
function lipSlope(h: number, d: number) {
  const v = 0.5 / CUT;
  const a = h / (v * v);
  return (2 * a * (1 - v)) / d;
}

function surfaceY(z: number, h: number, d: number) {
  const hd = d / 2;
  if (z <= hd) {
    const s = (z + hd) / d;
    return parabolaY(Math.min(1, Math.max(0, s)), h);
  }
  return parabolaY(1, h) + lipSlope(h, d) * (z - hd);
}

function thickAt(z: number, d: number) {
  const hd = d / 2;
  if (z <= hd) return THICK;
...
function buildHill(w: number, h: number, d: number) {
  ...
  for (let i = 0; i <= SEG; i++) {
    ...
    samples.push({ z, y: surfaceY(z, h, d), t: thickAt(z, d) });
  }
  ...
  for (let i = 0; i < COL; i++) {
    ...
    const za = z0 + u0 * span;
    const zb = z0 + u1 * span;
    const ya = surfaceY(za, h, d);
    const yb = surfaceY(zb, h, d);
    const ta = thickAt(za, d);
    const tb = thickAt(zb, d);
    hulls.push(
      new Float32Array([
        -hw, ya, za,
         hw, ya, za,
