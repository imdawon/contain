...
export function Wagon({
  ...
  useEffect(() => {
    registerBody(id, "wagon", () => poseOf(r.current, { grip: mu, cart: true }), () => r.current);
  ...
  return (
    <RigidBody
      ...
      ccd
    >
      <CuboidCollider args={[dw / 2, hitH, dd / 2]} collisionGroups={GROUPS} friction={mu} restitution={rest} />
      <mesh onPointerDown={grab.down} position={[0, wr - dh / 2, 0]}>
        <boxGeometry args={[dw, dh, dd]} />
        <meshStandardMaterial color={selected ? 0xd4d7cf : deckCol} roughness={0.82} metalness={0.06} />
      </mesh>
      <mesh position={[0, wr + 0.04, -dd / 2 + 0.04]}>
